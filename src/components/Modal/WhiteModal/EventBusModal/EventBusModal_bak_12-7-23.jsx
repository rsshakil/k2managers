...
const EventBusModal = ({
    ...
    useEffect(() => {
        ...
        Object.values(busTableData).map((busTimes, i) => {
            busTimes.forEach((bus) => {
                selectedBusWayIds.push(bus.busWayId);
                const time = Object.keys(busTableData)[i];
                bus.time = time;
                bus.busTime = Number(time.replace(':', ''));
        ...
        const selectedBusItems2 = [];
        for (let i = 0; i < selectedBusItems.length; i++) {
            let notmatchFlag = true;
            let row = {};
            for (let j = 0; j < busTableArr.length; j++) {
                if (selectedBusItems[i].busWayId == busTableArr[j].busWayId) {
                    row = busTableArr[j];
                    notmatchFlag = false;
                    break;
                }
            ...
            if (notmatchFlag) {
                selectedBusItems2.push(selectedBusItems[i]);
            }
            else {
                selectedBusItems2.push(row);
            }
        }
        setBusTableArr(selectedBusItems2);
...
// console.log("busReservationCount", item);
    ...
    const tagBoxSessionStorageKey = (sessionStorageKey, busTime) => {
        const eventBusSelectedItem = JSON.parse(sessionStorage.getItem(sessionStorageKey));

        const selectedBusArr = {};
        selectedBusArr[busTime] = [];
        busWayList.map((item, i) => {
            if (eventBusSelectedItem.includes(item.busWayId)) {
                item.busReservationCount = 0;
                selectedBusArr[busTime][i] = item;
            }
        });
        setBusWayList(busWayList);
        //setBusTableData(selectedBusArr);
        setBusTableData({ ...busTableData, ...selectedBusArr });
    };
