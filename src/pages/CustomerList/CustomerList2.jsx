...
const CustomerList = memo(() => {
    ...
    const getCustomerViewTemplate = async () => {
        ...
        try {
            ...
            const response = await instance.request(config);
            setTemplates(
                response?.data?.records?.map((template) => {
...
/*
            setCustomerViewTemplateCreateTemplateId(
                templateIdSession? 
                    response?.data?.records.find(template => template.customerViewTemplateId == templateIdSession).customerViewTemplateCreateTemplateId 
                    : response?.data?.records[0]?.customerViewTemplateCreateTemplateId
            );
*/
            sessionStorage.setItem('customer_view_template_' + projectId, JSON.stringify(response?.data?.records));
            let sessionTemplateListInfo = response?.data?.records;
            let firstDropdownTemplateId = response?.data?.records[0]?.customerViewTemplateId;
